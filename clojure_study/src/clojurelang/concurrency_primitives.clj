...
(let [watched-val (atom 0)
  ...
  (do
    ...
    (reset! watched-val 5)
    (a/assert= @copy-val-1 [:watch-1 watched-val 0 5])
    (a/assert= @copy-val-2 [:watch-2 watched-val 0 5])
...
(let [a-negative (agent -1)
  ...
  (do
    ...
    (Thread/sleep 100)
    (a/assert= -1 @a-negative)
    (a/assert= (first @err-collector) a-negative)
    (a/assert= IllegalStateException (type (second @err-collector)) )))
