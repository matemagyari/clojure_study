...
(let [action-logger (atom [])
  ...
  (a/assert-equals [2 3 4] (map logging-inc [1 2 3]))
  (a/assert-equals @action-logger [:m :m :m])
  (reset! action-logger [])
  (a/assert-equals [2 4] (filter logging-even? [1 2 3 4]))
  (a/assert-equals @action-logger [:f :f :f :f])
  ;filter then map on non-lazy sequence
  (reset! action-logger [])
  (a/assert-equals [3 5 7] (->> [1 2 3 4 5 6 7]
                             (filter logging-even?)
                             (map logging-inc)))
  (a/assert-equals @action-logger [:f :f :f :f :f :f :f :m :m :m])
  ;map then filter on non-lazy sequence
  (reset! action-logger [])
  (a/assert-equals [2 4 6 8] (->> [1 2 3 4 5 6 7]
                               (map logging-inc)
                               (filter logging-even?)))
  (a/assert-equals @action-logger [:m :m :m :m :m :m :m :f :f :f :f :f :f :f])
  ;map then filter on lazy sequence
  (reset! action-logger [])
  (a/assert-equals [2 4 6 8] (->> (range)
                               (map logging-inc)
                               (filter logging-even?)
                               (take 4)))
  ;(a/assert-equals @action-logger [:m :m :m :m :m :m :m :f :f :f :f :f :f :f])
