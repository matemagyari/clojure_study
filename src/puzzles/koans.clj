...
(assert= (my-fib-num 0) 0)
(assert= (my-fib-num 1) 0)
(assert= (my-fib-num 2) 1)
(assert= (my-fib-num 3) 1)
(assert= (my-fib-num 4) 2)
(assert= (my-fib-num 5) 3)
(assert= (my-fib-num 6) 5)
...
(assert= (my-fib 7) [1 1 2 3 5 8 13])
...
(assert= (get-caps "HeLlO, WoRlD!") "HLOWRD")
...
(assert= (apply str (my-comp "Leeeeeerrroyyy")) "Leroy")
...
(assert= (occ "abacbcccb") {\a 2 \b 3 \c 4})
...
(assert= (hex->dec "10") 16)
(assert= (hex->dec "A0") 160)
(assert= (hex->dec "AA") 170)
(assert= (hex->dec "ABC") 2748)
...
(assert= (rev-i (range 9) 3) [[0 3 6] [1 4 7] [2 5 8]])

;split-by-type
(defn splitter [a-seq]
  (->> a-seq (group-by type) vals))

(assert= (set (splitter [1 :a 2 :b 3 :c])) #{[1 2 3] [:a :b :c]})
...
(assert= (word-sorter "Have a nice day.")
  ["a" "day" "Have" "nice"])
...
(assert= ((my-comp rest reverse) [1 2 3 4 5]) [4 3 2 1])
...
(assert= (my-part 3 (range 8)) '((0 1 2) (3 4 5)))
...
(assert= (first-n-primes 5) [2 3 5 7 11])
(assert= (first-n-primes 1) [2])
