...
(defn init-story [persons rooms]
  (->> (r/empty-db) create-house (init-characters-positions persons rooms)))
...
(defn next-random-room [facts person]
  (let [[current-room time] (-> (path-of facts person) last)
        neighbours (neighbours facts current-room)
...
(defn memories-of [stories character]
  ...
  (let [with-character? (fn [room-time] ;room-time is a tuple of (characters room time)
                          (let [characters-in-the-room (first room-time)]
...
(defn theories [character facts]
  (with-db facts
    (run* [room-1 room-2 time-1 time-2]
      (r/present character room-1 time-1)
      (r/present character room-2 time-2)
